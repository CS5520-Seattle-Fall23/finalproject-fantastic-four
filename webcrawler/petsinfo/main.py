...
csv_file_path = 'realData_waitBuyLinkAndContent.csv'
...
for i in range(1, 11):
    driver.get(f'https://slickdeals.net/newsearch.php?page={i}&q=pet&searcharea=deals&searchin=first&isUserSearch=1')
    ...
    with open(csv_file_path, mode='a', encoding='utf-8', newline='') as f:
        ...
        for result in result_rows:
            ...
            thumbs_element = result.find_element(By.CSS_SELECTOR, 'div.ratingCol > div.ratingNum') if result.find_elements(
            ...
            dealLink = result.find_element(By.CSS_SELECTOR, 'div.dealImg > a').get_attribute('href') if result.find_element(
    ...
    if link:
        driver.get('https://slickdeals.net/newsearch.php?q=pet&searcharea=deals&searchin=first&isUserSearch=1')
        driver.implicitly_wait(5)
        
        csv_writer.writerow({'link': link})
    '''

    print(f'Collect Done{i}')
